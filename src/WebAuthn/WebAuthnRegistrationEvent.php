...
-----END CERTIFICATE-----";
        ...
        // I can't help but notice that the verification procedure does NOTHING with CA certs from the chain, nor is
        // there a root to validate to! Found the root CA with Google, see above, and will perform chain validation
        // even if the spec doesn't say so.
        ...
        foreach ($stmtDecoded['x5c'] as $runIndex => $runCert) {
            if (isset($stmtDecoded['x5c'][$runIndex + 1])) { // there is a next cert, so follow the chain
                ...
                if (openssl_x509_verify($certResource, $signerPubKey) != 1) {
                    $this->fail(
                        "Error during chain validation of the attestation certificate"
                        . " (while validating cert #$runIndex, which is "
        ...
        if ($keyResource === false) {
            $this->fail(
                "Did not get a parseable X.509 structure out of the Apple attestation statement - x5c nr."
                . " 0 statement was: XXX "
                . $stmtDecoded['x5c'][0]
        ...
        if ($this->AAGUIDDictionary->hasToken($this->AAGUID)) {
            ...
            if ($token['multi'] === true) { // need to check the OID
                if (
                    ...
                    $this->fail(
                        "This vendor uses one cert for multiple authenticator model attestations,"
                        . " but lacks the AAGUID OID."
                    );
                }
                /**
